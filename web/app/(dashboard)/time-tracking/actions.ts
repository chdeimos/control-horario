...
async function checkScheduleRestriction(userId: string, isClockIn: boolean) {
    ...
    // 1. First Priority: If no schedule record exists, it's not a workday
    if (!schedule) {
        return { error: 'No tienes una jornada laboral asignada para el día de hoy.' }
    }

    // 2. Second Priority: If schedule is flexible, we allow work (already checked that today is a workday)
...
export async function clockIn(lat: number, lng: number) {
    ...
    if (!lat || !lng) {
        return { error: 'La geolocalización es obligatoria para fichar desde el navegador.' }
    }
...
export async function clockOut(lat: number, lng: number) {
    ...
    if (!lat || !lng) {
        return { error: 'La geolocalización es obligatoria para cerrar la jornada desde el navegador.' }
    }
